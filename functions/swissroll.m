function [data, labels] = swissroll(N, centers, s, varargin)
    % File: swissroll.m
    % Author: Arthur Gay
    %
    ...
    p = inputParser;
    addRequired(p,'N');
    addRequired(p,'centers');
    addRequired(p,'s');
    addParameter(p, 'seed', 550);
    addParameter(p, 'output', 0);
    parse(p,N,centers,s,varargin{:});

    % setup
    close all
 
    % parameters
    seed = p.Results.seed; % repeatability
    %N = 500; % number of points in each class
    sx = s(1);  %2 standard deviation on x
    sy = s(2);  %3 standard deviation on y
